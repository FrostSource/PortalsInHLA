...
---
---Get the difference between two normalized vectors in degrees. This is always the smallest angle.
---
---@param vector Vector # Vector to check the angle between.
...
function meta:SignedAngleDiff(vector)
    local Ax = self.x
    local Ay = self.y
    local Bx = vector.x
    local By = vector.y
    -- Calculate the dot product
    local dot_product = Ax * Bx + Ay * By
    
    -- Determine the sign of the angle based on the orientation of the vectors
    local sign = 1
    if dot_product < 0 then
        sign = -1
    end

    -- The signed angle difference between perpendicular vectors is 90 degrees or -90 degrees
    local angle_degrees = sign * 90
